...
describe('BulkCreateWineService', () => {
  ...
  beforeEach(() => {
    ...
    bulkCreateWineService = new BulkCreateWineService(fakeWineRepository, fakeWineDataApiProvider);
  });

  it('should create a wine successfully with its price and properties', async () => {
    jest.spyOn(fakeWineDataApiProvider, 'getWinesData').mockImplementationOnce(async () => {
      return [
        {
          property: {
  ...
  it('should create a second price if the wine`s name be the same', async () => {
    ...
    const prices = await fakeWineRepository.findWinePricesById(result.wineCreated.id);
